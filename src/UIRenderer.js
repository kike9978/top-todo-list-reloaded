...
export default class UIRenderer {
  ...
  handleTodoChange(value, currentTask) {
    ...
    TaskService.updateTask(
      taskId,
      new Task(taskId, currentTask.getTitle(), value)
    );

    tasks = getCurrentProjectTasks().map(
      (d) => new Task(d.id, d.title, d.isCompleted)
    );
  ...
  displayTasksContainer(pendingTasks, completedTasks, taskList, projects) {
    ...
    const taskContainerInstance = new TasksContainer(
      ...
      projects,
      this.controller.handleDeleteListClick.bind(this.controller)
    );
  ...
  updateProjectsAndListsContainer() {
    this.sideMenuInstance.projectsAndListsContainerInstance.updateProjectsAndListsContainer(
      this.controller.projectsAndLists,
