...
export default class AppController {
  ...
  generateProjectAndListArr() {
    this.projectsAndLists = this.projectsAndListsOrder.map((item) => {
      if (item.type === "project") {
        ...
        const lists = project.assignedListIds.map((listId) =>
          this.taskListService.getListById(listId)
        );
        lists.forEach((list) => this.setTaskListPendingTasksCount(list.id));
        const updatedList = project.assignedListIds.map((listId) =>
          this.taskListService.getListById(listId)
        );
  ...
  controlTaskDisplay() {
    if (
      ...
      this.view.displayTasksContainer(
        ...
        {
          title: this.taskListService.getListById(this.currentTaskListId).title,
  ...
  getCurrentTasksListTasksSeparated() {
    const currentTaskListsTasksIds = this.taskListService
      .getListById(this.currentTaskListId)
      .getAssignedTasksIds();
  ...
  addNewTask(taskData, taskListId) {
    const itAlreadyHasTasks =
      this.taskListService.getListById(taskListId).assignedTasksIds.length > 0;
    this.taskService.createTask(taskData);
    this.taskListService.addTaskToList(taskData.id, taskListId);
    const currentTaskListTasksIds = this.taskListService.getTaskListTasksIds(
      this.currentTaskListId
    );
    const currentTaskListTasks = this.taskService.getTaskListTasks(
      currentTaskListTasksIds
    );
  ...
  getCurrentProjectTasks() {
    const currentProjectTasksIds =
      this.projectService.getCurrentProjectTasksIds(this.currentProjectId);
  ...
  handleUpdateListSubmit(listAndProjectAssignationData) {
    const updatedList = this.taskListService.getListById(
      listAndProjectAssignationData.id
    );
    ...
    const isChangingFromNone = this.projectsAndListsOrder.some((item) => {
      return (
        item.id === listAndProjectAssignationData.id && item.type === "taskList"
      );
    });
    const isChangingToNone =
      listAndProjectAssignationData.assignedProjectId === "none";

    if (isChangingFromNone) {
      this.removeItemFromProjectsAndListsOrder(
        "taskList",
        listAndProjectAssignationData.id
      );
    } else {
      this.projectService.removeTaskListfromProject(
        listAndProjectAssignationData.id
      );
    }

    if (!isChangingToNone) {
      this.projectService.addTaskListToProject(
        listAndProjectAssignationData.id,
        listAndProjectAssignationData.assignedProjectId
      );
    } else {
      this.projectsAndListsOrder = [
        ...this.projectsAndListsOrder,
  ...
  handleDeleteListClick(listId) {
    ...
    const nextTaskList = this.projectsAndListsOrder.find(
