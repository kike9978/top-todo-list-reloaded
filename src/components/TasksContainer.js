...
export default class TasksContainer {
  constructor(
    ...
    const parentProject = this.projects.find((project) =>
      project.assignedListIds.includes(this.taskListId)
    );
  ...
  createTaskContainer() {
    ...
    this.mainTasksContainer.className =
      "basis-full overflow-y-auto flex flex-col";
  ...
  createEditTaskListForm() {
    ...
    this.projects.forEach((project) => {
      form.querySelector("select").innerHTML += `
        ...
        this.parentProjectId === project.id && "selected"
    ...
    form.addEventListener("submit", (e) => {
      ...
      if (
        newListData.title === this.taskListTitle &&
        newListData.assignedProjectId === this.parentProjectId
      ) {
  ...
  displayPendingTasks(pendingTasks) {
    ...
    pendingTasks.forEach((task) => {
      pendingTasksContainer.appendChild(
        TodoItem(task, this.handleTodoChange, this.handleDeleteTaskClick)
      );
    });
  }
