export default function TodoItem(task, onChange, handleDeleteTaskClick) {
    ...
    article.innerHTML = `
        ...
        <div class="flex items-center gap-2 w-full">
            <input type="checkbox" ${task.isCompleted ? "checked" : ""} " class="appearance-none border-solid border-2  border-blue-400 size-6 rounded-full cursor-pointer inline-grid place-content-center before:content-[''] before:size-3 before:bg-blue-400 before:scale-0 before:transition-transform before:rounded-full hover:before:scale-100 hover:before:bg-blue-300 checked:before:scale-100 "> 
            
                   <div class="w-11/12">
               <p class="${task.isCompleted ? "line-through text-slate-400" : ""}"> ${task.title}</p>
               <p class="text-slate-500 text-xs truncate">${task.description}</p>
                   </div>
        </div>
