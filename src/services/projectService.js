...
export default class ProjectService {
  constructor() {
    ...
    this.myProjects = JSON.parse(localStorage.getItem("projects"))
      ? JSON.parse(localStorage.getItem("projects")).map(
          (project) =>
            new Project({
              assignedListIds: project.assignedListIds,
              id: project.id,
              title: project.title,
            })
        )
  ...
  populate() {
    this.myProjects = JSON.parse(localStorage.getItem("projects")).map(
      (project) =>
        new Project({
          assignedListIds: project.assignedListIds,
          id: project.id,
  ...
  updateProjectTitle(projectId, newTitle) {
    ...
    const updatedProject = new Project({
      id: projectId,
      title: newTitle,
      assignedListIds: projectToUpdate.assignedListIds,
    });
  ...
  addTaskListToProject(listId, projectId) {
    const nextProjects = this.myProjects.map((project) => {
      if (
        ...
        return new Project({
          ...project,
          assignedListIds: [...project.assignedListIds, listId],
        });
      } else {
        return project;
      }
    ...
    const nextProjects = this.myProjects.map((project) => {
      if (project.assignedListIds.includes(listId)) {
        const newProject = new Project({
          ...project,
          assignedListIds: project.assignedListIds.filter(
            (id) => id !== listId
          ),
        });
        return newProject;
