...
export default class TaskService {
  constructor() {
    ...
    this.myTodos = JSON.parse(localStorage.getItem("tasks"))
      ? JSON.parse(localStorage.getItem("tasks")).map((task) => new Task(task))
      : [];
  }

  populate() {
    this.myTodos = JSON.parse(localStorage.getItem("tasks")).map(
      (task) => new Task(task)
    );
  }
